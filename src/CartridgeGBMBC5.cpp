...
uint32_t CartridgeGBMBC5::readROM(void *buf_, uint32_t size, uint32_t offset){
uint8_t *buf = (uint8_t*)buf_;
  // Implementation specific to MBC5
  ...
  gb_write_byte(0x3000, 0x0);

  return readSize;
}

uint32_t CartridgeGBMBC5::readRAM(void *buf_, uint32_t size, uint32_t offset){
  uint8_t *buf = (uint8_t*)buf_;
...
uint32_t CartridgeGBMBC5::writeRAM(const void *buf_, uint32_t size, uint32_t offset){
